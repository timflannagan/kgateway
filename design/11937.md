...
This proposal favors deterministic fallback in the translator rather than pausing xDS updates or dropping the entire listener. Fallback provides clear failure semantics without halting all configuration updates to the proxy, which is especially important in dynamic environments like Kubernetes where endpoint sets change frequently.

See the [Alternatives](#alternatives) section for more details on the different approaches considered.

#### Operational Impact

- Route configuration failures (Gateway-wide or HTTPS listener attachment)
  - Replace the entire route configuration with a synthetic virtual host
  - The synthetic vhost should use domains: ["*"] and contain a single catch-all route (prefix: "/") that returns an HTTP 500 direct response
  - All previously computed vhosts are removed to ensure consistent fail-closed behavior across all hostnames
  - Avoids any shadowing issues that would occur if a catch-all vhost co-existed with other valid vhosts
- Virtual host failures (HTTP listener attachment)
  - Replace the affected virtual host with a synthetic vhost that preserves the original domain list
  - The replacement vhost contains a single catch-all route that returns HTTP 500 for all requests
  - This is more efficient and operationally clearer than attempting to replace each computed route individually, which could give the false impression that individual routes were invalid
- Observability
