...
func TestAgwRouteCollectionTimeouts(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gl := GatewayListener{
				Config: &Config{Meta: Meta{Name: "test-gateway", Namespace: "default"}},
				Parent: ParentKey{Kind: wellknown.GatewayGVK, Name: "test-gateway", Namespace: "default"},
				ParentInfo: ParentInfo{
					InternalName: "default/test-gateway",
					Protocol:     gwv1.HTTPProtocolType,
					Port:         80,
			...
			agwRoutes := AgwRouteCollection(httpRoutes, grpcRoutes, tcpRoutes, tlsRoutes, routeInputs, krtutil.KrtOptions{})
			agwRoutes.WaitUntilSynced(context.Background().Done())

			// Assert results
			results := agwRoutes.List()
			require.Len(t, results, 1, "expected exactly one route result")
			require.NotNil(t, results[0].Resources)
			rr := results[0].Resources[0].GetRoute()
