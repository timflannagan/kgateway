...
// TrafficPolicyGatewayExtensionIR represents the IR for a TrafficPolicyGatewayExtension.
// Several policy types rely on a backing server that is configured via a GatewayExtension.
...
// Equals returns true if the two TrafficPolicyGatewayExtensionIR objects are equal.
