...
func (h *httpRouteConfigurationTranslator) envoyRoutes(
	...
	if err != nil {
		// TODO(tim): what's the right conditions to set here? if runRoutePlugins returns an error, then
		// we always treat that as terminal, set the condition on the route, and replace with a 500.
	...
	// If we get here, the route is valid, so clear any previous error conditions
	routeReport.SetCondition(reportssdk.RouteCondition{
